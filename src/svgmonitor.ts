class SVGMonitor extends HTMLElement {
    ...
    private _svgRootPromise: Promise<Document>;
    private _svgRoot: Document | undefined;
    private _text: string;
    private _sleeper: Function;
    constructor() 
    {
        super();
        ...
        this._svgRootPromise = new Promise<Document>((resolve, reject) => {
    ...
    async TypeAdd(message:string, newline:boolean = true, delay: number=150) {
        if(newline) message = '\n'+message;
    ...
    async TypeBackspace(chars:number, delay:number = 150){
    ...
    async TypeOut(message: string, delay:number = 150) {
        var startIndex =0;
    ...
    get BSOD(): boolean {
        if (this._svgRoot === undefined) return false;
    ...
    set ScreenText(value: string) {
        ...
        for (var ln = firstLine; ln < lines.length; ++ln) {
            ...
            let span = <SVGTSpanElement><unknown>this._svgRoot.createElementNS('http://www.w3.org/2000/svg', "tspan");

            let spang = <SVGTSpanElement><unknown>this._svgRoot.createElementNS('http://www.w3.org/2000/svg', "tspan");
